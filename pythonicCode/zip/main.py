...
Implement the following functions using enumerate():
    ...
    1. group_names_and_scores(names: List[str], scores: List[int]) -> Dict[str, int] that takes two lists, names and scores, and returns a dictionary where the key is names[i] and it maps to scores[i] as the value.
